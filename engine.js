    ...
    shuffledShorts.forEach((vid) => {
        ...
        reel.innerHTML = `
            ...
            <div class="side-actions">
                ...
                <div class="action-item" onclick="openMoviesList()"><i class="fa-solid fa-film action-icon"></i><span class="action-text">Movies</span></div>
            </div>
        `;
...
document.addEventListener('touchend', e => {
    ...
    if (tStart - tEnd > 70) openMoviesList(); 
    if (tEnd - tStart > 70) closeSidebars(); 
});
...
function closeSidebars() { document.getElementById('leftSidebar').classList.remove('open'); document.getElementById('rightMoviesSidebar').classList.remove('open'); document.getElementById('sidebarOverlay').style.display = 'none'; }
...
function renderMoviesGrid() {
    ...
    if (movies.length === 0) {
        grid.innerHTML = `<div class="no-results" style="grid-column: 1/-1;"><i class="fa-solid fa-film"></i><p>No movies found</p></div>`;
        return;
    }
    
    grid.innerHTML = movies.map(v => `
        <div class="grid-item" onclick="openMoviePlayer('${v.id}')">
            <img src="${v.thumbnail}">
            <div class="views"><i class="fa-solid fa-play"></i> Watch</div>
            <div class="movie-title-overlay">${v.title}</div>
        </div>
    `).join('');
}
